...
if EID then
	Astro:AddEIDCollectible(
		...
		"",
		"...",
		"↑ {{Tears}} 연사 x0.8#공격 시 10%의 확률로 눈물 주변에 레이저가 생깁니다.#중첩 시 기본 확률이 합 연산으로 증가하고 쿨타임이 줄어듭니다.#!!! {{LuckSmall}}행운 수치 비례: 행운 90 이상일 때 100% 확률 (행운 1당 +1%p)"
	)
end
...
local function TryRandom(player)
	...
	if data["technologyOmicronCooldown"] == nil or data["technologyOmicronCooldown"] < Game():GetFrameCount() then
		local rng = player:GetCollectibleRNG(Astro.Collectible.TECHNOLOGY_OMICRON)
		local num = player:GetCollectibleNum(Astro.Collectible.TECHNOLOGY_OMICRON)
...
local function RealFireTear(tear, player)
	if player:HasCollectible(Astro.Collectible.TECHNOLOGY_OMICRON) and ShouldTearGetEffects(tear) and TryRandom(player) then
...
local function PostThrowKnife(knife, player)
	if player:HasCollectible(Astro.Collectible.TECHNOLOGY_OMICRON) and TryRandom(player) then -- or mod.HasPridePinEffect(player, mod.PridePinEffect.TECH_OMICRON) then
...
Astro:AddCallback(
	...
	function(_, laser)
		...
		if player and player:HasCollectible(Astro.Collectible.TECHNOLOGY_OMICRON) and TryRandom(player) then
			laser.Variant = 9
